...
function tryUseAbility(queue, target, spellName, delay)
if queue == true then
	windower.chat.input:schedule(delay, '/ja "' .. string.trim(spellName) .. '" ' .. target .. '')
	tickdelay = os.clock() + delay + 0.25
	return false
	else
		windower.chat.input('/ja "' .. string.trim(spellName) .. '" ' .. target .. '')
		return true
	end
end
